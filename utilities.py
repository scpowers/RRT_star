...
def steer(near_node: XYThetaNode, rand_node: XYThetaNode):
    ...
    if dist < MIN_STEP:
        return None
    # if the heading difference is too large, change heading as much as you can in the correct direction
    new_heading = rand_node.yaw
    if abs(new_heading - near_node.yaw) > YAW_DIFF_THRESHOLD:
        new_heading = near_node.yaw + YAW_DIFF_THRESHOLD*np.sign(new_heading - near_node.yaw)
